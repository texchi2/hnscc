...
# Abstract {#abstract}
  ...
  Gene or protein expression data are usually represented by metric or at least ordinal variables. In order to translate a continuous variable into a clinical decision, it is necessary to determine a cutoff point and to stratify patients into two groups each requiring a different kind of treatment.
	...
	This protocol describes using R Markdown and RStudio, user-friendly tools for statistical analysis and reproducible research in bioinformatics, to analyze and document analysis of an RNA-Seq data set of TCGA cohort, which were suffered from HNSCC.\cite{Loraine2015}
...
---
target journal: https://cancersheadneck.biomedcentral.com/submission-guidelines?utm_source=hybris&utm_medium=email&utm_content=internal&utm_campaign=BSCH_2_AC_BSCH-BMC-41199-CHEN&sap-outbound-id=9F691E593C1E47A120B7CD007507CA4FE0DD5BF4
BBRC: Biochemical and Biophysical Research Communications https://www.journals.elsevier.com/biochemical-and-biophysical-research-communications
https://bmcbioinformatics.biomedcentral.com 
BMC Bioinformatics, impact factor 2 (https://www.letpub.com/index.php?page=journalapp&view=detail&journalid=1243&xuanxiangk_id=2)
...
set up a data science environment on Google Cloud Platform (GCP)
it is also fine: 
# [FirebrowseR] data preparation {#sec:2}
...
#analysis based on gene expression data? Well, let's say you have a number of GDCRNATools;
...
7)[COXPH modelling] with OS, RFS; HNSCC features: pathologic to clinical TNM
rename LUAD.mRNA.Exp.
export to .Rda and .XLS (appending is better) HNSCC_survivalAnalysis_marginS_
...
OR (marinS or marginFree on main_) on main_marginS_free_HNSCC.R 手動選擇:-O
.....


run mainB (marginFree)
call function(survival_marginFree) on  TCGA_HNSCC_marginFree.R, rename LUAD.mRNA.Exp., load(file="LUAD_T522_clinical_fullFeatures11_dichotomized.Rda")
core engine: cutofFinder_func_HNSCC.R, call, run100
{
7)[COXPH modelling] with OS, RFS; ; HNSCC features: pathologic to clinical TNM
rename LUAD.mRNA.Exp.
export to .XLS (it could not be appended)
export part I:
# P-value notes:
# Significant codes:  0 ???***??? 0.001 ???**??? 0.01 ???*??? 0.05 ???.??? 0.1 ??? ??? 1
# if <0.001 => mark as "***"
export part II: R2Excel export
{
 Table 2: clinical correlation tables (tableChi1)
 table 3 and table 4 
 plot: Kaplan-Meier survival estimate with optimized Cutoff
}
...
HNSCC_survivalAnalysis_marginPlus_
...
# as 5.31011e-06 (2019 run01)
# } Bonferroni end
turn the "number" frequency to z-scores ("Z" because the normal distribution is also known as the "Z distribution").
a candidate table (HNSCC_OS_marginS_pvalue1e_6_zscore0_6), with "number of P-value" under cutoff finding (becoming z-score: bigger, much more significant cutoff "sites")
...
# ***after Bonferroni correction => n=? in _marginFree_

post2 process: add table2 (new variable: KM + Cox): candidate_cox
Pickup all significant genes list -> HNSCC_OS_marginS_THREE_pvalue005 (n=1408, KM P-value only (NOT by Bonferroni_cutoff))
...
#pickup1 n=11 from HNSCC_OS_marginS_THREE_pvalue005; 
# Cox HR (>=2.5) (bad guy genes) (Bonferroni_cutoff = 5.31011e-06 is too restricted in this scenario)
# & (uni_P_value <= 0.05) & (multi_P_value <= 0.05)
c pickup2 from HNSCC_OS_marginS_THREE_pvalue005
#* Cox HR <=0.5 (good_FC) (good guy genes) (Bonferroni_cutoff = 5.31011e-06 is too restricted in this scenario)
venn diagram for intersection of them:
candidate genes: bad buy 7 and good guy 13
#
#Export r2excel and .Rda
The z-score summary plot: HNSCC_OS_marginS_candidates_Venn.xlsx was exported successfully. 
...
TMU-biobank@tmu.edu.tw
***published # Excluding the HNSCC cancer driver genes list??? ##
#BioXpress*.csv # data from https://hive.biochemistry.gwu.edu/cgi-bin/prd/bioxpress/servlet.cgi
***and TMU biobank (Oral cavity= 39:25) T/N specific gene signature (交集 with my gene cluster)
