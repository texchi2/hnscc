...
Table 3: Cox proportion hazard regression analysis, univariate and multipvariate => OS hazard ratio (HR), Cox P-value and its 95% confidence interval (CI)
...
{
Table 1: candidate genes by KM adjusted P-value (why?  Bonferroni correction is NOT necessary) => Z-score
and its scatter plot
Table 2A and 2B and 2C: bad guy candidate genes => KM P-value, Cox P-value and HR (>=2.5)
and its venn diagram
}
how to z-score
no need Bonferroni
how about false discovery rate (FDR)?
...
post2 process: add table2 (new variable: KM + Cox): which yield n=6601 genes on the list.
candidate_cox
...
#***publication # Excluding the HNSCC cancer driver genes list??? ##
[GDC data portal]
